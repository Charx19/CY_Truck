#include <stdio.h>
	...
	int departMarseille(Ville* racine)
	{
		if(racine == NULL)
			return 0;
		if (strcmp("MARSEILLE", racine->nom) < 0)
			return departMarseille(racine->gauche);
		else if (strcmp("MARSEILLE", racine->nom) > 0)
			return departMarseille(racine->droite);
		else {
			return racine->departs;
		}
	}
	...
	char** ligneToTab(char* l, char* type)
	{
		...
		*(res + idx) = 0;
		...
		// Fonction pour parcourir l'arbre et prendre le nom des villes ayant le plus de trajets
	void prendreVille(Ville* racine, int compte, char ListeDeNom[10][TAILLE], int ListeDeTrajets[]){
		int condition = 0;
		if (racine != NULL) {
			prendreVille(racine->gauche, compte, ListeDeNom, ListeDeTrajets);
			...
			prendreVille(racine->droite, compte, ListeDeNom, ListeDeTrajets);
		}
	}
	
	void creationFichier(FILE* fichier, Ville* racine, char nom[10][TAILLE]) 
	{
		if(racine != NULL)
		{
			creationFichier(fichier, racine->gauche, nom);
			
			for(int i = 0; i < 10; i++){
				if(!strcmp(nom[i], racine->nom))
				{
					fprintf(fichier, "%s;%d;%d\n", racine->nom, racine->trajetsTotals, racine->departs);
				}						
			}
			
			creationFichier(fichier, racine->droite, nom);			
		}
		
	}
	
	int creationFichierAppel(Ville* racine)
	{
		// Ouvrir le fichier en mode écriture ("w")
		FILE *fichier = fopen("T4.txt", "w");

		// Vérifier si le fichier a pu être ouvert avec succès
		if (fichier == NULL) {
			printf("Erreur lors de l'ouverture du fichier.\n");
			return 0;  // Quitter le programme avec un code d'erreur
		}
		
		// Cherche les villes à écrire dans le fichier
		char ListeDeNom[10][TAILLE];
		int ListeDeTrajets[10];
		...
		for(int i = 0; i < 10; i++)
		{
			strcpy(ListeDeNom[i], racine->nom);
			ListeDeTrajets[i] = racine->trajetsTotals;
			prendreVille(racine, i, ListeDeNom, ListeDeTrajets);
		}

		// Ecrit les villes dans le fichier
		fprintf(fichier, "Ville;Total;Départs\n");
		creationFichier(fichier, racine, ListeDeNom);
