	...
	void creationFichier(Route* racine, FILE* fichier)
	{
		if(racine!=NULL)
		{
			creationFichier(racine->gauche, fichier);
			
			// Écrire des lignes dans le fichier
			fprintf(fichier, "%d;%f;%f;%f\n", racine->id, racine->max, racine->min, racine->moyenne);
			
			creationFichier(racine->droite, fichier);
		}
	}


	int creationFichierAppel(Route* racine)
	{
		// Ouvrir le fichier en mode écriture ("w")
		FILE *fichier = fopen("S4.txt", "w");
		...
		fprintf(fichier, "IdRoute;Max;Min;Moyenne\n");
		
		creationFichier(racine, fichier);
